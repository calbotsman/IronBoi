...
export function assembleCoachPrompt(
  ...
  const system = [
    ...
    "- An exercise marked like '(+5 lb/wk)' progresses automatically: each week the app raises its weight by that step from the weight the user last worked at, up to a cap. That is why a number is higher than last week — say so when asked. If they're missing reps, say it's too hard, or are coming back from time off, propose a hold or deload with adapt_plan rather than letting the step keep climbing.",
